...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($title) || empty($slug) || empty($content)) {
        ...
        try {
            $stmt = $conn->prepare("INSERT INTO articles (title, slug, content, excerpt, status, author_id, published_at) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $published_at = $status === 'published' ? date('Y-m-d H:i:s') : null;
            $stmt->bind_param("sssssis", $title, $slug, $content, $excerpt, $status, $author_id, $published_at);
            $stmt->execute();
            $success = '文章添加成功';
            // 清空表单
            $title = $slug = $content = $excerpt = '';
        } catch(Exception $e) {
            $error = '添加文章时出错: ' . $e->getMessage();
        }
    }
}
?>
            ...
            <div class="form-container">
                <form method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <div class="slug-preview">预览: <a href="#" id="slug-preview-link">/article/<?php echo isset($slug) ? htmlspecialchars($slug) : 'slug'; ?></a></div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="excerpt" name="excerpt" rows="3"><?php echo isset($excerpt) ? htmlspecialchars($excerpt) : ''; ?></textarea>
                    </div>
